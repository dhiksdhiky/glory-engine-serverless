...
// Navigation state (for future extensions)
...
[uploadModal, suspendModal, logsModal].forEach(modal => {
    if(modal) {
...
function renderDashboard() {
    ...
    appContent.innerHTML = `
        <div class="dashboard-grid">
            <!-- Data Overview (Combined Master & DB Stats) -->
            <div class="glass-card" style="grid-column: 1 / -1;">
                <div class="card-header" style="margin-bottom: 15px;">
                    ...
                    <span class="status-badge badge-green">Last Synced: ${master.last_update}</span>
                </div>
                
                <div class="stat-grid" style="grid-template-columns: repeat(3, 1fr); gap: 10px;">
                    <div class="stat-box">
                        <div class="stat-box-title" style="font-size: 0.7rem;">Total Emiten</div>
                        <div class="stat-box-value" style="font-size: 1.2rem;">${master.total}</div>
                    </div>
                    <div class="stat-box">
                        <div class="stat-box-title" style="font-size: 0.7rem;">Harga Aktif</div>
                        <div class="stat-box-value" style="font-size: 1.2rem;">${(archiveInfo.stats.total_harga || 0).toLocaleString('id-ID')}</div>
                    </div>
                    <div class="stat-box">
                        <div class="stat-box-title" style="font-size: 0.7rem;">Broksum Aktif</div>
                        <div class="stat-box-value" style="font-size: 1.2rem;">${(archiveInfo.stats.total_broksum || 0).toLocaleString('id-ID')}</div>
                    </div>
                </div>
            ...
            <!-- Month Calendar Heatmap Card (STANDOUT) -->
            <div class="glass-card" style="grid-column: 1 / -1; border: 1px solid var(--accent-emerald); background: rgba(16, 185, 129, 0.05); box-shadow: 0 4px 15px rgba(0, 0, 0, 0.3);">
                <div class="card-header" style="margin-bottom: 15px;">
                    <div class="card-title" style="color: var(--accent-emerald);">
                        ...
                        <span id="calendar-month-title" style="font-weight: 600;">Calendar Sync History</span>
                    </div>
                </div>
            ...
            <!-- Action Bar (Static Buttons) -->
            <div style="grid-column: 1 / -1; display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px;">
                <button id="btnActionSuspend" class="glass-card" style="padding: 12px; display: flex; flex-direction: column; align-items: center; justify-content: center; border: 1px solid rgba(255,255,255,0.15); cursor: pointer; background: var(--bg-panel); color: var(--text-main); position: relative; border-radius: 12px; gap: 8px; transition: background 0.2s; overflow: visible;" onmouseover="this.style.background='rgba(255,255,255,0.1)'" onmouseout="this.style.background='var(--bg-panel)'">
                    ${gapsList.length > 0 ? `<div style="position: absolute; top: -6px; right: -6px; background: var(--accent-cyan); color: #000; font-size: 0.75rem; font-weight: bold; width: 20px; height: 20px; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 5px rgba(0,0,0,0.5);">${gapsList.length}</div>` : ''}
                    <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--accent-cyan)" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>
                    <span style="font-size: 0.75rem;">Info Suspensi</span>
                </button>
                <button id="btnActionLogs" class="glass-card" style="padding: 12px; display: flex; flex-direction: column; align-items: center; justify-content: center; border: 1px solid rgba(255,255,255,0.15); cursor: pointer; background: var(--bg-panel); color: var(--text-main); position: relative; border-radius: 12px; gap: 8px; transition: background 0.2s; overflow: visible;" onmouseover="this.style.background='rgba(255,255,255,0.1)'" onmouseout="this.style.background='var(--bg-panel)'">
                    ${data.errors && data.errors.length > 0 ? `<div style="position: absolute; top: -6px; right: -6px; background: var(--accent-rose); color: #fff; font-size: 0.75rem; font-weight: bold; width: 20px; height: 20px; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 5px rgba(0,0,0,0.5);">${data.errors.length}</div>` : ''}
                    <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--accent-rose)" stroke-width="2"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"></rect><line x1="8" y1="21" x2="16" y2="21"></line><line x1="12" y1="17" x2="12" y2="21"></line></svg>
                    <span style="font-size: 0.75rem;">Bot Issues</span>
                </button>
                <button id="btnActionUpload" class="glass-card" style="padding: 12px; display: flex; flex-direction: column; align-items: center; justify-content: center; border: 1px solid var(--accent-emerald); cursor: pointer; background: var(--accent-emerald); color: #fff; border-radius: 12px; gap: 8px; transition: filter 0.2s; overflow: visible;" onmouseover="this.style.filter='brightness(1.1)'" onmouseout="this.style.filter='brightness(1)'">
                    <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="17 8 12 3 7 8"></polyline><line x1="12" y1="3" x2="12" y2="15"></line></svg>
                    <span style="font-size: 0.75rem; font-weight: 500;">Upload Master</span>
                </button>
            </div>
        </div>
    `;
    ...
    const suspendContentEl = document.getElementById('suspendModalContent');
    const logsContentEl = document.getElementById('logsModalContent');
    
    if (suspendContentEl) {
        suspendContentEl.innerHTML = gapsHtml;
    }
    if (logsContentEl) {
        logsContentEl.innerHTML = errorsHtml;
    }
    
    // Attach event listeners to the new static buttons
    document.getElementById('btnActionUpload')?.addEventListener('click', () => {
        document.getElementById('uploadModal').style.display = 'flex';
    });
    document.getElementById('btnActionSuspend')?.addEventListener('click', () => {
        document.getElementById('suspendModal').style.display = 'flex';
    });
    document.getElementById('btnActionLogs')?.addEventListener('click', () => {
